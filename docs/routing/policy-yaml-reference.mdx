...
| `max_input_tokens` | Integer | No | Maximum estimated text-input tokens per request |
| `images_max` | Integer | No | Maximum images per request |
| `admit_budget_tokens` | Integer | No | Token-weighted in-flight budget for a model |
| `max_inflight` | Integer | No | In-flight request-count limit for a model |
...
The admission fields are top-level properties of the policy, not fields inside `routing_policy`.
...
The request path enforces `max_input_tokens`, `images_max`, `admit_budget_tokens`, and `max_inflight` for the LLM passthrough endpoints. The effective occupancy budget is scaled by `HIVENET_ROUTER_ADMIT_FRACTION`, and over-budget requests may park for `HIVENET_ROUTER_ADMIT_PARK_TIMEOUT` before the router returns `429 concurrency_limit_exceeded`.
...
See [Admission control](/routing/admission-control) for request-footprint calculation, parking, error responses, and the current serverless-mode boundary.
