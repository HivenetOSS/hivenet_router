...
description: "Understand how Hivenet Router filters candidates, gates on health and capacity, ranks agents, retries, queues, and falls back across inference agents."
...
<Note>
  For streaming responses, Hivenet Router holds the agent capacity slot until the stream fully drains, whether the stream ends normally or the client disconnects. The active-request count therefore includes ongoing streams routed through Hivenet Router, and capacity gates apply for the whole generation.
</Note>
...
Because streaming requests hold their agent slot until the stream fully drains, an agent serving active streams has a higher load ratio than an idle one. `least-loaded` therefore spreads concurrent streaming traffic across agents instead of repeatedly selecting the first agent in peer-ID order.
