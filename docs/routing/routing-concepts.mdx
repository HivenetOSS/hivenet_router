...
Mycoroute routes requests through a 3-layer policy pipeline, with fallback chains for high availability.

## The 3-Layer Pipeline

Every request passes through three sequential layers, with a fallback waterfall when the primary pool is exhausted:

![Mycoroute Policy Funnel and Fallback Waterfall](../images/Policy.png)
...
`fallback_provider` is a top-level field (not a step inside `fallback_chain`). It is invoked as the last resort after all `fallback_chain` steps are exhausted. Set `mycoroute_OPENAI_API_KEY` or `mycoroute_ANTHROPIC_API_KEY` in the environment to supply credentials.
...
- [Fallback Chains](/routing/fallback-chains) - Detailed fallback configuration
- [Provider Fallback](/routing/provider-fallback) - OpenAI/Anthropic integration
- [Detailed Architecture](/reference/detailed-architecture) - Request lifecycle
