...
<Note>
A route with a single signal scores `1.0` whenever that signal matches. If one weak signal should not be enough, add a second signal or set `min_score`.
</Note>
...
The conversation switches when the newly chosen route scores at least `affinity_margin` more than its current route. It also moves on when its model is no longer eligible (for example, no healthy agent), or when a reload removes that model from the route. Affinity entries share the pin store and its limits. A route with `pin_task` uses its task pin instead.
