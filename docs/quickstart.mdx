...
```bash
scp bin/hivenet-router router-server:/opt/mycoroute/
```

Copy the agent binary to each inference host:

```bash
scp bin/hivenet-agent gpu-eu-1:/opt/mycoroute/
scp bin/hivenet-agent gpu-eu-2:/opt/mycoroute/
scp bin/hivenet-agent gpu-us-1:/opt/mycoroute/
```
