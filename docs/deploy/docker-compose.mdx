...
```yaml
- HIVENET_ROUTER_JWT_SECRET=${MYCOROUTE_JWT_SECRET:?Set MYCOROUTE_JWT_SECRET}
```
...
environment:
  ...
  - HIVENET_ROUTER_ADMIN_API_KEYS=${MYCOROUTE_ADMIN_API_KEYS:?Set MYCOROUTE_ADMIN_API_KEYS}
```
...
docker compose exec prometheus \
  wget -qO- http://localhost:9090/use-the-api/v1/targets \
...
docker compose exec prometheus \
  wget -qO- \
  'http://localhost:9090/use-the-api/v1/query?query=hivenet_router_routing_agent_info' \
  | jq .
```
