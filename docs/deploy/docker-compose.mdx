...
description: "Use Docker Compose to run the Mycoroute router with its full observability stack, then connect agents from separate inference hosts."
sidebarTitle: "Docker Compose"
---

Use Docker Compose to run the Mycoroute router with its full observability stack, then connect agents from separate inference hosts.
...
| `docker-compose.agent.yml` | Each inference host | One Mycoroute agent |
...
| --- | --- | --- |
| Mycoroute router | API, agent authentication, and request routing | `8888`, `8902`, `8903` |
...
| Service | Purpose | Exposed port |
| --- | --- | --- |
| Mycoroute agent | Connects the backend to the router | Outbound connection only |
...
```yaml
- mycoroute_JWT_SECRET=changeme
```

Replace it with:

```yaml
- mycoroute_JWT_SECRET=${MYCOROUTE_JWT_SECRET:?Set MYCOROUTE_JWT_SECRET}
```

The names use different capitalization deliberately:

- `MYCOROUTE_JWT_SECRET` is the host-side variable read by Docker Compose.
- `mycoroute_JWT_SECRET` is the environment variable read inside the Mycoroute container.

Create a `.env` file on the router host:

```bash
printf 'MYCOROUTE_JWT_SECRET=%s\n' "$(openssl rand -hex 32)" > .env
chmod 600 .env
```
...
<Steps>
  ...
  <Step title="Copy the shared secret">
    ...
    ```bash
    scp router-server:/path/to/mycoroute/.env .
    chmod 600 .env
    ```
  ...
  <Step title="Update and start the agent">
    ...
    The agent authenticates through gRPC, receives the router’s libp2p connection details, and initiates the persistent transport connection. It needs outbound access to router ports `8902` and `8903`; it does not expose an inbound Mycoroute port.
  </Step>
...
| `MYCOROUTE_JWT_SECRET` | Yes | None | Shared router-agent secret used during Compose substitution |
...
Mycoroute uses least-loaded routing without a policy file by default.
...
environment:
  ...
  - mycoroute_ADMIN_API_KEYS=${MYCOROUTE_ADMIN_API_KEYS:?Set MYCOROUTE_ADMIN_API_KEYS}
```

Add the corresponding host-side value to `.env`:

```text
MYCOROUTE_ADMIN_API_KEYS=<secure-admin-key>
```
...
```yaml
- mycoroute_QUOTA_BACKEND=badger
```
...
The default volume names assume the Compose project is named `mycoroute`. Confirm the actual names first:

```bash
docker volume ls \
  --filter label=com.docker.compose.project=mycoroute
```
...
```bash
docker run --rm \
  -v mycoroute_badger_data:/data:ro \
  -v "$PWD/backup:/backup" \
  alpine \
...
```bash
docker run --rm \
  -v mycoroute_prometheus_data:/data:ro \
  -v "$PWD/backup:/backup" \
  alpine \
...
```bash
docker run --rm \
  -v mycoroute_grafana_data:/data:ro \
  -v "$PWD/backup:/backup" \
  alpine \
...
Confirm that Prometheus contains Mycoroute data:

```bash
docker compose exec prometheus \
  wget -qO- \
  'http://localhost:9090/api/v1/query?query=mycoroute_routing_agent_info' \
  | jq .
```
