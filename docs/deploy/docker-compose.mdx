---
title: "Docker Compose — Full Observability Stack"
---
...
> **GPU host preparation:** before starting agents, run `setup-agent-host.sh` on each GPU server as described in [Docker Quickstart — Step 0](/deploy/docker-quickstart#step-0-prepare-each-gpu-server). That script installs Docker and the NVIDIA Container Toolkit.
...
│  Router Server                                                  │
│                                                                 │
│  mycoroute/router   :8888  HTTP API (clients)                  │
...
│  GPU Server                                                     │
│                                                                 │
│  mycoroute/agent    outbound libp2p → router (:8903)            │
│                                                                 │
│  vLLM / Ollama / …   :8888  inference backend (host, not Docker)│
└─────────────────────────────────────────────────────────────────┘
```

The agent connects outbound to the router (`ROUTER_GRPC`, `ROUTER_P2P`), and the router forwards inference requests back over that same libp2p connection. The agent needs no inbound ports, no public IP, and no firewall rules — it works behind NAT and Docker with zero network configuration. See [the connection flow in the quickstart](/deploy/docker-quickstart#connection-flow) for details.
...
MYCOROUTE_JWT_SECRET="$(openssl rand -hex 32)" docker compose up -d
...
MYCOROUTE_JWT_SECRET="$(openssl rand -hex 32)" \
  docker compose up -d
```
...
scp router-server:/path/to/mycoroute/docker-compose.agent.yml .
...
MYCOROUTE_JWT_SECRET=<same-secret-as-router> \
...
| `MYCOROUTE_JWT_SECRET` | Yes | Same secret as the router |
...
```yaml
environment:
  - mycoroute_AUTH_MODE=dynamic
  - mycoroute_ADMIN_API_KEYS=your-admin-key-here   # required in dynamic mode
```

No `auth.yaml` needed. Keys are managed via `POST /admin/api-keys/replace`. `mycoroute_ADMIN_API_KEYS` is mandatory: the router auto-elevates admin auth to `api-key` so the key-management endpoints aren't public, and startup fails if it's missing. See [API Keys — Dynamic Key Mode](/security/api-keys#dynamic-key-mode).
...
```promql
# Active agents
count(mycoroute_routing_agent_info)
...
rate(mycoroute_routing_requests_routed_total[5m])
  / (rate(mycoroute_routing_requests_routed_total[5m]) + rate(mycoroute_routing_requests_failed_total[5m]))

# P90 SRTT across fleet
histogram_quantile(0.9, sum by (le) (rate(mycoroute_agent_srtt_ms_bucket[5m])))
```

### Grafana Dashboards

Pre-provisioned dashboards:
- **Mycoroute Router** — agent health, request counters, SRTT, hardware metrics, policy routing
- **Mycoroute Audit** — audit log viewer with tenant/model/status filters (Loki-backed)
...
docker run --rm \
  -v mycoroute_badger_data:/data \
  -v $(pwd)/backup:/backup \
  alpine tar czf /backup/badger-$(date +%Y%m%d).tar.gz /data

docker run --rm \
  -v mycoroute_prometheus_data:/data \
  -v $(pwd)/backup:/backup \
  alpine tar czf /backup/prometheus-$(date +%Y%m%d).tar.gz /data

docker run --rm \
  -v mycoroute_grafana_data:/data \
  -v $(pwd)/backup:/backup \
...
docker compose exec prometheus wget -qO- \
  'http://localhost:9090/api/v1/query?query=mycoroute_routing_agent_info' | jq .
...
- [Bare-Metal Deployment](/deploy/bare-metal) — Running binaries directly without Docker
- [Agent Deployment](/deploy/agents/vllm) — vLLM, Ollama, llama.cpp, SGLang configurations
- [Prometheus Metrics](/observability/prometheus-metrics) — Full metric reference
- [Grafana Dashboards](/observability/grafana-dashboards) — Dashboard setup and customisation
