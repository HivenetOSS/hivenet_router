...
Deploy a Mycoroute agent connected to an SGLang inference backend.
...
> **`--enable-metrics` is required** for Mycoroute to scrape KV cache and request-queue metrics from SGLang.
...
Mycoroute scrapes these SGLang metrics every 500 ms (requires `--enable-metrics`). All metrics are aggregated at the router's Prometheus endpoint (`:2112`). Agents have no separate Prometheus port.
...
- [vLLM Agent](/deploy/agents/vllm) — vLLM backend
