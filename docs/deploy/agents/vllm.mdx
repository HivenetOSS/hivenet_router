...
Deploy a Mycoroute agent connected to a vLLM inference backend.
...
./bin/mycoroute-agent --model meta-llama/Llama-3.1-8B-Instruct --engine vllm ...

# Separate agent for Llama-3.1-70B (different process, different --identity-path)
./bin/mycoroute-agent --model meta-llama/Llama-3.1-70B-Instruct --engine vllm ...
```
...
Mycoroute scrapes these vLLM metrics every 500 ms and aggregates them at the router's Prometheus endpoint (`:2112`). Agents have no separate Prometheus port.
...
- [Ollama Agent](/deploy/agents/ollama) — Ollama backend setup
- [SGLang Agent](/deploy/agents/sglang) — SGLang backend setup
- [Routing Concepts](/routing/routing-concepts) — Configure routing policies
- [Hardware Metrics](/observability/hardware-metrics) — GPU monitoring
