---
title: "Ollama agent"
description: "Connect an Ollama backend to Hivenet Router, select the model an agent represents, and configure capacity for local or edge inference."
sidebarTitle: "Ollama"
---

Connect an Ollama server to Hivenet Router by running an agent beside the backend.

The agent checks Ollama through its `/use-the-api/tags` endpoint, registers one available model with the router, and forwards chat-completion requests to Ollama’s OpenAI-compatible API.
...
```bash
curl http://localhost:11434/use-the-api/tags \
  | jq .
```
...
```text
GET <backend-url>/use-the-api/tags
```
...
When `--model` is omitted, the agent registers the first model returned by `/use-the-api/tags`.
