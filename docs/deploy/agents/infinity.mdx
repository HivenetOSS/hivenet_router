...
description: "Connect Infinity embedding and reranking models to Mycoroute by running one capability-specific agent for each model."
sidebarTitle: "Infinity"
---

Connect an Infinity server to Mycoroute for embedding and reranking workloads.

Infinity can serve several models from one process. Mycoroute connects to that process through separate agents, with one agent representing one model and one capability.
...
Mycoroute expects Infinity to expose:
...
Set `INFINITY_URL_PREFIX=/v1` so the inference and model endpoints use the paths expected by Mycoroute.
...
The Mycoroute agent and API clients use:
...
<Tabs>
  ...
  <Tab title="Bare metal">
    ...
    ```bash
    sudo mkdir -p /var/lib/mycoroute/agents
    sudo chown "$USER":"$USER" /var/lib/mycoroute/agents
    ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Docker">
    ```bash
    docker run -d \
      --name mycoroute-embedding-agent \
      --restart unless-stopped \
      --network host \
      --gpus all \
  ...
  <Tab title="Bare metal">
    ...
    ./bin/mycoroute-agent \
      ...
      --region EU-France \
      --tags infinity,embedding \
      --identity-path /var/lib/mycoroute/agents/embedding.key
    ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Docker">
    ```bash
    docker run -d \
      --name mycoroute-reranking-agent \
      --restart unless-stopped \
      --network host \
      --gpus all \
      -v /opt/mycoroute/jwt.secret:/jwt.secret:ro \
  ...
  <Tab title="Bare metal">
    ...
    ./bin/mycoroute-agent \
      ...
      --region EU-France \
      --tags infinity,reranking \
      --identity-path /var/lib/mycoroute/agents/reranking.key
    ```
  </Tab>
</Tabs>
...
Mycoroute uses the agent capability to separate different kinds of inference traffic.

| Capability | Mycoroute endpoint | Infinity endpoint |
| --- | --- | --- |
...
Do not leave the default capability of `llm` on an Infinity agent. Infinity rejects chat-completion requests, and Mycoroute reports a structured invalid-request error.

## Set agent capacity

`--capacity` controls how many concurrent requests Mycoroute may assign to an agent.
...
<Note>
  For streaming-capable workloads, Mycoroute releases an agent capacity slot when response headers arrive, while backend work can continue. Infinity embeddings and reranking are normally non-streaming, but the same `--capacity` value remains a routing-admission setting rather than a direct backend scheduler limit.
</Note>
...
Mycoroute routes embedding and reranking requests through the same policy pipeline used for language-model requests:
...
```promql
rate(
  mycoroute_routing_requests_routed_total{
    engine="infinity",
    model="bge-m3"
  }[5m]
)
...
Without the prefix, Infinity serves its API at different paths from those expected by Mycoroute.
...
<Tabs>
  <Tab title="Docker">
    ```bash
    docker logs mycoroute-embedding-agent
    docker logs mycoroute-reranking-agent
    ```
  </Tab>
  <Tab title="systemd">
    ```bash
...
```bash
docker inspect mycoroute-embedding-agent \
  | jq '.[0].Mounts'
```
...
<CardGroup cols={3}>
  <Card title="Custom engine agent" href="/deploy/agents/custom-engine">
    Connect another OpenAI-compatible backend to Mycoroute.
  </Card>
  ...
  <Card title="Reranking" href="/api/reranking">
