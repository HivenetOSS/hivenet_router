...
Deploy a Mycoroute agent connected to an [Infinity](https://github.com/michaelfeil/infinity) embedding and reranking server.
...
./bin/mycoroute-agent \
  ...
  --identity-path      /opt/mycoroute/agent_embed.key
```
...
./bin/mycoroute-agent \
  ...
  --capacity           32 \
  --region             EU-Reranking \
  --identity-path      /opt/mycoroute/agent_reranker.key
```
...
rate(mycoroute_routing_requests_routed_total{model="bge-m3"}[1h])

# Reranking request rate
rate(mycoroute_routing_requests_routed_total{model="bge-reranker-large"}[1h])

# SRTT per Infinity agent
mycoroute_agent_srtt_ms{engine="infinity"}
```
