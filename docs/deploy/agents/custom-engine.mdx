...
description: "Connect an OpenAI-compatible chat-completion backend to Mycoroute using an explicit model name and health endpoint."
sidebarTitle: "Custom engine"
---

Connect another OpenAI-compatible inference server to Mycoroute with the `custom` engine.

Use this integration when your backend is not covered by one of Mycoroute’s built-in engine types but exposes a compatible chat-completions endpoint.
...
Mycoroute forwards the original chat-completion request body and HTTP headers to the backend. It does not translate a proprietary request format into the OpenAI schema.
...
Because the Mycoroute agent runs beside the backend, the backend can remain bound to localhost unless another machine must reach it directly.
...
Mycoroute considers the backend ready only when this endpoint returns HTTP `200`.
...
Confirm this request works before starting the Mycoroute agent.
...
| `--model` | Explicit model name registered with Mycoroute |
...
Backend errors are also passed back through Mycoroute. Where possible, Mycoroute classifies common errors such as:
...
The backend must provide a valid streaming response. Mycoroute does not convert a non-streaming backend response into SSE.
...
When the agent reaches capacity, Mycoroute considers another matching agent, waits if queueing is configured, or moves through the policy’s fallback chain.

<Note>
  For streaming responses, Mycoroute releases the agent capacity slot when response headers arrive, while backend generation can continue. Treat `--capacity` as a routing-admission setting rather than a hard limit on ongoing streams, and verify the custom backend under sustained streaming load.
</Note>
...
Send a request through Mycoroute:
...
Mycoroute can relay an SSE stream, but it cannot correct buffering introduced by the backend or an intermediate proxy.
...
<CardGroup cols={3}>
  <Card title="Chat completions" href="/api/chat-completions">
