---
title: "Audit logging"
description: "Record structured Hivenet Router request metadata, ship audit files through Promtail and Loki, and correlate requests with logs, metrics, and traces."
...
| `alias` | Present only on [semantic alias](/routing/semantic-routing) requests: the alias the client named. `model` holds the resolved model, or the alias when resolution failed |
| `semantic_route` | Present only on semantic alias requests: the route that chose the model; empty when resolution failed |
