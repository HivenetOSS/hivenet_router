...
description: "Understand how Hivenet Router measures router-observed request latency and calculates smoothed per-agent SRTT and RTTVAR values for routing."
...
The router’s `--max-concurrent` forwarding slot is released at this response-header boundary. The agent’s declared capacity slot is not. Hivenet Router holds the agent capacity slot until the stream fully drains, whether the stream ends normally or the client disconnects. The SRTT sample is recorded at stream start, but the agent’s active-request count covers the whole generation.
