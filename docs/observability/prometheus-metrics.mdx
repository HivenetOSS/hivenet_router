...
Mycoroute exports comprehensive metrics via Prometheus for monitoring and alerting.
...
mycoroute_routing_agent_info{peer_id="...", region="EU", engine="vllm", model="...", capacity="10", organization="acme", machine="node-1"}

# Agent health: 1=healthy, 0=unhealthy
mycoroute_routing_agent_healthy{peer_id="...", region="EU", engine="vllm", model="...", capacity="10", organization="acme", machine="node-1"}

# Timestamp of last heartbeat (Unix milliseconds)
mycoroute_routing_agent_last_seen_timestamp{peer_id="...", region="EU", engine="vllm", model="...", capacity="10", organization="acme", machine="node-1"}
...
mycoroute_routing_requests_routed_total{model="meta-llama/Llama-3.1-8B-Instruct"}

# Requests that could not be forwarded
mycoroute_routing_requests_failed_total{engine="vllm"}
...
mycoroute_agent_requests_success_total{peer_id="12D3Koo0..."}

# Lifetime failed requests per agent
mycoroute_agent_requests_failed_total{peer_id="12D3Koo0..."}

# Derived success rate (0–1.0)
mycoroute_agent_success_rate{peer_id="12D3Koo0..."}

# Real-time capacity utilization (active_requests / capacity)
mycoroute_agent_capacity_utilization{peer_id="12D3Koo0..."}

# Smoothed RTT in milliseconds (RFC 6298)
mycoroute_agent_srtt_ms{peer_id="12D3Koo0..."}

# RTT variance in milliseconds (RFC 6298)
mycoroute_agent_rttvar_ms{peer_id="12D3Koo0..."}

# Lifetime input (prompt) tokens
mycoroute_agent_input_tokens_total{peer_id="12D3Koo0..."}

# Lifetime output (completion) tokens
mycoroute_agent_output_tokens_total{peer_id="12D3Koo0..."}

# Times agent was at full capacity (slot rejected)
mycoroute_agent_rejected_requests_total{peer_id="12D3Koo0..."}

# Lifetime disconnections
mycoroute_agent_disconnections_total{peer_id="12D3Koo0..."}

# Agent marked unhealthy by health monitor (missed heartbeats)
mycoroute_agent_failure_total{peer_id="12D3Koo0..."}

# Agent reported its backend as unhealthy via heartbeat
mycoroute_model_backend_failure_total{peer_id="12D3Koo0..."}
```

> These per-agent counters are **persisted to BadgerDB** (`universalHistory:`) and re-seeded into Prometheus when an agent re-registers, so they survive a router restart. To clear them after a deploy and observe a change against a clean baseline, call `POST /admin/metrics/reset` (see [Admin Endpoints](/api/admin-endpoints#reset-per-agent-lifetime-counters)).
...
mycoroute_agent_engine_request_success_total{peer_id="...", finished_reason="stop"}

# Token generation throughput (llama.cpp only)
mycoroute_agent_engine_predicted_tps{peer_id="..."}
mycoroute_agent_engine_prompt_tps{peer_id="..."}
```
...
histogram_quantile(0.90, sum by (le) (rate(mycoroute_agent_engine_ttft_seconds_bucket[5m])))

# ITL latency histogram
histogram_quantile(0.90, sum by (le) (rate(mycoroute_agent_engine_itl_seconds_bucket[5m])))

# Per-request prompt token length distribution (workload shape)
histogram_quantile(0.90, sum by (le) (rate(mycoroute_agent_engine_request_prompt_tokens_bucket[5m])))

# Per-request generation token length distribution (workload shape)
histogram_quantile(0.90, sum by (le) (rate(mycoroute_agent_engine_request_generation_tokens_bucket[5m])))
```
...
mycoroute_agent_gpu_utilization_percent{peer_id="...", gpu_index="0"}

# VRAM in use (bytes)
mycoroute_agent_gpu_vram_used_bytes{peer_id="...", gpu_index="0"}

# VRAM free (bytes)
mycoroute_agent_gpu_vram_free_bytes{peer_id="...", gpu_index="0"}

# Total VRAM (bytes)
mycoroute_agent_gpu_vram_total_bytes{peer_id="...", gpu_index="0"}

# GPU temperature in Celsius
mycoroute_agent_gpu_temperature_celsius{peer_id="...", gpu_index="0"}

# GPU power draw in watts
mycoroute_agent_gpu_power_watts{peer_id="...", gpu_index="0"}
```
...
mycoroute_agent_cpu_usage_percent{peer_id="..."}

# System memory used percent (0–100)
mycoroute_agent_memory_used_percent{peer_id="..."}

# System memory available (bytes)
mycoroute_agent_memory_available_bytes{peer_id="..."}

# Total system memory (bytes)
mycoroute_agent_memory_total_bytes{peer_id="..."}
```
...
mycoroute_policy_primary_routed_total{model="meta-llama/Llama-3.1-8B-Instruct"}

# Requests served by any fallback_chain step
mycoroute_policy_fallback_routed_total{model="meta-llama/Llama-3.1-8B-Instruct"}

# Requests served by fallback_provider (closed-source API)
mycoroute_policy_provider_fallback_total{model="meta-llama/Llama-3.1-8B-Instruct"}

# Requests where all policy steps exhausted (client got 503)
mycoroute_policy_exhausted_total{model="meta-llama/Llama-3.1-8B-Instruct"}
...
mycoroute_agent_connection_resets_total{model="meta-llama/Llama-3.1-8B-Instruct", reason="forward_failure"}
...
mycoroute_queue_depth{model="meta-llama/Llama-3.1-8B-Instruct"}

# Time spent in queue before dispatch (histogram)
histogram_quantile(0.95, rate(mycoroute_queue_wait_seconds_bucket[5m]))
```
...
| `deployment_id` | Agent registration metadata (`mycoroute_DEPLOYMENT_ID` env on the agent pod) | `"unset"` for pre-routing failures (auth, rate-limit, no agent available) or agents that predate this field |
...
mycoroute_tenant_requests_success_total{tenant_id="acme", key_id="key-abc", deployment_id="dep-1", model="..."}

# Requests failed per tenant / key / deployment
mycoroute_tenant_requests_failed_total{tenant_id="acme", key_id="key-abc", deployment_id="dep-1", model="..."}

# Requests rejected by RPM quota (key_id only — deployment context not relevant for rate limiting)
mycoroute_tenant_rate_limited_total{tenant_id="acme", key_id="key-abc"}

# Requests rejected by daily token budget (phase=input|output)
mycoroute_tenant_token_limited_total{tenant_id="acme", key_id="key-abc", deployment_id="dep-1", phase="input"}

# Lifetime prompt tokens per tenant / key / deployment
mycoroute_tenant_input_tokens_total{tenant_id="acme", key_id="key-abc", deployment_id="dep-1", model="..."}

# Lifetime completion tokens per tenant / key / deployment
mycoroute_tenant_output_tokens_total{tenant_id="acme", key_id="key-abc", deployment_id="dep-1", model="..."}

# Tokens consumed today (UTC calendar day; resets at midnight) — tenant-level only
mycoroute_tenant_tokens_used_today{tenant_id="acme"}

# Configured RPM limit (0 = unlimited) — tenant-level only
mycoroute_tenant_quota_rpm_limit{tenant_id="acme"}

# Configured daily token limit (0 = unlimited) — tenant-level only
mycoroute_tenant_quota_tpd_limit{tenant_id="acme"}

# Unix timestamp (seconds) of the most recent request — enables "Last used" per key
mycoroute_tenant_last_request_timestamp{tenant_id="acme", key_id="key-abc", deployment_id="dep-1"}

# End-to-end request duration per tenant / key / deployment (histogram)
histogram_quantile(0.95, rate(mycoroute_tenant_request_duration_seconds_bucket{tenant_id="acme", key_id="key-abc", deployment_id="dep-1"}[5m]))

# Redis quota backend call failures (fail-open events)
mycoroute_quota_backend_errors_total
```
...
sum by (deployment_id) (
  increase(mycoroute_tenant_requests_success_total{tenant_id="acme"}[7d])
)

# Last-used timestamp per API key
max by (key_id) (mycoroute_tenant_last_request_timestamp{tenant_id="acme"})

# Token consumption breakdown by key
sum by (key_id, model) (mycoroute_tenant_input_tokens_total{tenant_id="acme"})
```
...
histogram_quantile(0.95, rate(mycoroute_request_duration_seconds_bucket[5m]))
```
...
- alert: HighFailureRate
  expr: >
    rate(mycoroute_routing_requests_failed_total[5m]) /
    (rate(mycoroute_routing_requests_routed_total[5m]) + rate(mycoroute_routing_requests_failed_total[5m])) > 0.1
  for: 5m
  labels:
    severity: warning
  annotations:
...
```yaml
- alert: AgentDown
  expr: mycoroute_routing_agent_healthy == 0
  for: 1m
  labels:
    severity: warning
  annotations:
...
- alert: HighAgentLatency
  expr: mycoroute_agent_srtt_ms > 5000
  for: 5m
  labels:
    severity: warning
  annotations:
...
- **Mycoroute Router** — Agent health, request metrics, policy routing
- **Mycoroute Audit** — Request logs (requires Loki)
...
- [Audit Logging](/observability/audit-logging) - Request logging
- [Detailed Architecture](/reference/detailed-architecture) - Metric tier system
