...
`route` and `source` are empty when no model was chosen. `source` is one of `pinned`, `scored`, `default`, `fallback`, or `count`. `outcome` is one of `ok`, `invalid`, `forbidden`, `too_long`, `unsupported`, or `unavailable`. Label values come from your alias configuration and fixed sets, so cardinality is bounded by the alias YAML.
