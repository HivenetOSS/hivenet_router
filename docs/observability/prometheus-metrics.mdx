...
description: "Scrape Hivenet Router metrics for routing, agents, engines, hardware, policies, queues, tenants, quotas, and HTTP traffic."
...
```yaml
      - alert: MycorouteHighRoutingFailureRate
...
```yaml
      - alert: MycorouteKVCachePressure
...
```yaml
      - alert: MycorouteGPUHighTemperature
...
```yaml
      - alert: MycoroutePolicyExhaustion
...
```yaml
      - alert: MycorouteQuotaPersistenceFailure
...
```yaml
groups:
  - name: mycoroute-recording

    interval: 30s

    rules:
      - record: mycoroute:model_request_rate5m
      ...
      - record: mycoroute:model_failure_rate5m
      ...
      - record: mycoroute:model_p95_ttft_seconds5m
