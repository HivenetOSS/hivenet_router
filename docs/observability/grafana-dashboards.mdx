...
| Hivenet Router — Tenant Quota | `mycoroute-tenants` | Prometheus | Tenant activity, quotas, tokens, and latency |
| Hivenet Router Audit | `mycoroute-audit` | Loki | Structured request records and errors |
...
```text
http://<router-host>:3000/d/mycoroute-tenants/
```
...
docker compose exec prometheus \
  wget -qO- \
  http://localhost:9090/use-the-api/v1/targets \
