...
Streaming requests hold the agent slot until the stream fully drains, so this panel covers the whole generation for both streaming and non-streaming traffic. Compare it with engine running and waiting requests, KV-cache pressure, TTFT, and ITL to check that the declared capacity matches what the engine can sustain.
