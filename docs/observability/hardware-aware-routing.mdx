...
description: "Route across GPU hardware tiers and exclude Hivenet Router agents under GPU, memory, CPU, cache, thermal, or latency pressure automatically."
...
<Note>
  ...
  Streaming requests hold the declared capacity slot until the stream fully drains, so capacity-based gates account for the whole generation. Use engine running and waiting requests, KV-cache pressure, TTFT, and ITL alongside capacity-based gates to catch GPU pressure that declared slots do not reflect.
</Note>
