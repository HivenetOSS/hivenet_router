...
For streaming responses, the selected agent’s capacity slot remains occupied until the stream fully drains. The router releases it when the stream ends normally or when the client disconnects. Active-request counts, capacity utilization, capacity gates, and `least-loaded` ranking therefore cover the whole streaming generation.

The router’s `--max-concurrent` forwarding slot is released earlier, after response headers arrive and streaming begins.
