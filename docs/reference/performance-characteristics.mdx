...
Hivenet Router holds the selected agent’s declared capacity slot for the complete generation in both response modes:

- For a non-streaming response, the slot is held until the complete response has been received and parsed.
- For a streaming response, the slot is held until the stream fully drains. Hivenet Router releases it when the stream ends normally or when the client disconnects.

As a result, `active_requests`, `hivenet_agent_capacity_utilization`, the hard capacity gate, `capacity_utilization` policy gates, and `least-loaded` ranking cover the whole streaming generation. This matches how inference engines count running sequences, such as vLLM’s `--max-num-seqs`.

The router’s `--max-concurrent` forwarding slot behaves differently. For a streaming request, Hivenet Router releases it after response headers arrive and streaming begins. The global forwarding semaphore therefore limits concurrent dispatches, not concurrent active streams.

<Note>
  Long streams now occupy agent capacity for their full duration. A fleet dominated by long streams reaches its declared agent capacity sooner than it did when slots were released at stream start. Requests that find every eligible agent full wait in the per-model capacity queue.
</Note>

For streaming-heavy workloads:

- size agent `--capacity` to the number of concurrent sequences the engine can run well, for example close to vLLM’s `--max-num-seqs`
...
- size the per-model capacity queue (`--queue-depth`) and router request timeout for the longer slot hold times of streaming requests

The inference engine’s own scheduler metrics remain the authoritative view of backend work, including traffic that does not pass through Hivenet Router.
...
For streaming requests, it is released when streaming begins. The selected agent’s capacity slot is held separately until the stream fully drains.
...
For streaming workloads, compare Hivenet Router’s agent active-request count with engine-level running and waiting requests. Both cover the full generation, so a persistent gap usually points to a capacity mismatch or to traffic that bypasses the router.
...
| Low Hivenet Router capacity use but high engine running requests | Agent `--capacity` versus engine limits, direct engine traffic | Capacity mismatch or traffic outside the router |
