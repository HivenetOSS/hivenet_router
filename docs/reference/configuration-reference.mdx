...
| `--backend-api-key-file` | `HIVENET_ROUTER_BACKEND_API_KEY` | Empty | API key sent as `Authorization: Bearer` on every request to the backend (health, model discovery, metrics, inference), replacing the client's credentials; any `X-Api-Key` header is removed. The flag reads the key from a file and wins over the environment variable, which holds the raw value. Empty forwards the client's headers unchanged. See [Use a vLLM server that requires an API key](/deploy/agents/vllm#use-a-vllm-server-that-requires-an-api-key) |
