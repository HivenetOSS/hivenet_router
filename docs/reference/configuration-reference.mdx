...
| `--semantic-decision-log` | `HIVENET_ROUTER_SEMANTIC_DECISION_LOG` | Empty | JSONL file that records every [semantic alias](/routing/semantic-routing) decision; empty disables it |
| `--semantic-decision-log-buffer` | `HIVENET_ROUTER_SEMANTIC_DECISION_LOG_BUFFER` | `4096` | Decision records queued before new ones are dropped |
| `--semantic-pin-max` | `HIVENET_ROUTER_SEMANTIC_PIN_MAX` | `100000` | Maximum semantic task pins held in memory; least recently used pins are evicted beyond it |
| `--semantic-pin-max-per-key` | `HIVENET_ROUTER_SEMANTIC_PIN_MAX_PER_KEY` | `10000` | Maximum semantic task pins per API key |
...
| `--semantic-decision-log` | `HIVENET_ROUTER_SEMANTIC_DECISION_LOG` | Empty |
| `--semantic-decision-log-buffer` | `HIVENET_ROUTER_SEMANTIC_DECISION_LOG_BUFFER` | `4096` |
| `--semantic-pin-max` | `HIVENET_ROUTER_SEMANTIC_PIN_MAX` | `100000` |
| `--semantic-pin-max-per-key` | `HIVENET_ROUTER_SEMANTIC_PIN_MAX_PER_KEY` | `10000` |
...
| `--backend-api-key-file` | `HIVENET_ROUTER_BACKEND_API_KEY` | Empty | API key sent as `Authorization: Bearer` on every backend request, replacing the client's credentials (any `x-api-key` header is removed). The flag reads the key from a file; the environment variable holds the raw value. Empty forwards the client's headers unchanged |
...
| `--backend-api-key-file` | `HIVENET_ROUTER_BACKEND_API_KEY` | Empty |
