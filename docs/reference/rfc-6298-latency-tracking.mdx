---
title: "RFC 6298 Latency Tracking"
---

Mycoroute measures per-agent latency using an asymmetric EWMA adapted from RFC 6298, the TCP retransmission timeout algorithm. The result is a Smoothed RTT (SRTT) that drives both routing policy gates and future latency-based selection strategies.
...
avg by (region) (mycoroute_agent_srtt_ms)

# Agents with high SRTT (>500ms)
mycoroute_agent_srtt_ms > 500
...
GOLOG_LOG_LEVEL=debug ./bin/mycoroute-router

# Only the router and metrics subsystems
GOLOG_LOG_LEVEL=router=debug,metrics=debug ./bin/mycoroute-router
```

Then watch the RTT update lines:

```bash
journalctl -fu mycoroute-router | grep -i srtt
```

## See Also

- [Detailed Architecture](/reference/detailed-architecture) - Storage architecture
- [Performance Characteristics](/reference/performance-characteristics) - Timing targets
