...
/var/lib/mycoroute/agent-identity.key
```
...
| `--agent-addr-ttl` | — | `2m` | Lifetime of an advertised agent address in the router peerstore |
...
| `HIVENET_ROUTER_AUDIT_LOG_PATH` | `/var/log/mycoroute/audit.jsonl` | Structured audit-log path |
| `HIVENET_ROUTER_MAX_REQUEST_BYTES` | `10485760` | Maximum request body size for `/v1/*` endpoints in bytes; `0` disables the limit |
...
```bash
export HIVENET_ROUTER_AUDIT_LOG_PATH=/var/log/mycoroute/audit.jsonl
```
...
./bin/hivenet-agent \
  ...
  --jwt-secret-file \
    /etc/hivenet-router/jwt.secret \
  --identity-path \
    /var/lib/mycoroute/custom-agent-identity.key
```
