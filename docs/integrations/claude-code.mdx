...
Claude Code sends requests to the routerâ€™s:
...
- vLLMâ€™s `--served-model-name`
- the Hivenet Router agentâ€™s registered model
- the Claude Code model mapping
- the API keyâ€™s model restrictions or per-model quotas
...
- be included in the keyâ€™s model restrictions or per-model quotas
...
Claude Code
  â†’ reverse proxy
  â†’ Hivenet Router router
  â†’ Hivenet Router agent
  â†’ inference backend
```
...
Do not begin by pinning an old Claude Code release. First confirm whether the backendâ€™s current Anthropic Messages implementation accepts the system-content shape the client sends.
...
Inspect the backendâ€™s first error.
