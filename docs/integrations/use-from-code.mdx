...
```bash
export MYCOROUTE_URL="https://router.example.com"
export MYCOROUTE_API_KEY="<mycoroute-api-key>"
export MYCOROUTE_MODEL="<registered-model-id>"
```

The examples construct the endpoint appropriate to each SDK:

- OpenAI-compatible clients use `${MYCOROUTE_URL}/v1`
- Anthropic-compatible clients use `MYCOROUTE_URL`
...
```bash
curl -X POST \
  "$MYCOROUTE_URL/v1/chat/completions" \
  -H "Authorization: Bearer $MYCOROUTE_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{
    \"model\": \"$MYCOROUTE_MODEL\",
    \"messages\": [
      {
        \"role\": \"user\",
...
router_url = os.environ["MYCOROUTE_URL"].rstrip("/")
api_key = os.environ["MYCOROUTE_API_KEY"]
model = os.environ["MYCOROUTE_MODEL"]
...
const routerURL = process.env.MYCOROUTE_URL?.replace(/\/+$/, "");
const apiKey = process.env.MYCOROUTE_API_KEY;
const model = process.env.MYCOROUTE_MODEL;

if (!routerURL || !apiKey || !model) {
  throw new Error(
    "MYCOROUTE_URL, MYCOROUTE_API_KEY, and MYCOROUTE_MODEL are required.",
  );
}
...
client = Anthropic(
    base_url=router_url,
    api_key=None,
    auth_token=os.environ["MYCOROUTE_API_KEY"],
    max_retries=0,
)
```
