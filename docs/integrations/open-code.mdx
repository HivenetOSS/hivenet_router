...
```bash
curl \
  -H "Authorization: Bearer <mycoroute-api-key>" \
...
vllm serve <model-source> \
  --host 0.0.0.0 \
  --port 8888 \
  --served-model-name mycoroute-code-model \
...
```json
{
  ...
  "model": "mycoroute/mycoroute-code-model",
  "small_model": "mycoroute/mycoroute-code-model",
...
- `mycoroute-code-model` with an exact model ID from `/v1/models`
...
```bash
export MYCOROUTE_API_KEY="<mycoroute-api-key>"
```

Verify that the variable exists without printing its value:

```bash
test -n "$MYCOROUTE_API_KEY" \
  && echo "MYCOROUTE_API_KEY is set"
```
...
```http
Authorization: Bearer <mycoroute-api-key>
```

This is the authentication header Hivenet Router expects.

<Note>
  `MYCOROUTE_API_KEY` is an environment variable used by the OpenCode client.

  It does not need to follow the lower-case `mycoroute_*` convention used by the Hivenet Router router process.
</Note>
...
```json
{
  "options": {
    "baseURL": "https://router.example.com/v1",
    "apiKey": "{file:~/.secrets/mycoroute-api-key}"
  }
}
```
...
`mycoroute` is the OpenCode provider ID.
...
```json
{
  ...
  "model": "mycoroute/code-model-large",
  "small_model": "mycoroute/code-model-fast",
...
```text
mycoroute/code-model-large
mycoroute/code-model-fast
```
...
opencode debug config \
  | jq '{
      ...
      share,
      permission,
      mycoroute: .provider.mycoroute
    }'
```

Confirm that:

- `mycoroute` appears in `enabled_providers`
- `model` and `small_model` use the `mycoroute/` prefix
- `baseURL` ends in `/v1`
...
```bash
opencode models mycoroute
```

A result may resemble:

```text
mycoroute/mycoroute-code-model
```
...
```bash
opencode run \
  -m mycoroute/mycoroute-code-model \
...
```text
Create a file named mycoroute-test.txt containing the word connected.
```
...
- the JSON is valid
- `mycoroute` is included in `enabled_providers`
...
- `MYCOROUTE_API_KEY` contains the raw client key
