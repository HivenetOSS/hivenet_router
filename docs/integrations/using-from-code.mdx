---
title: "Using Mycoroute from code"
---

This guide shows how to call a Mycoroute router from your own scripts and services using the standard OpenAI / Anthropic SDKs and common frameworks (LangChain, etc.). It targets app developers integrating the router into code (not interactive terminal users).
...
- A reachable Mycoroute router and an API key.
- One of: **Python ≥ 3.8** (for the Python SDKs) or **Node.js ≥ 18** (for the JS SDKs). Anything more recent works.
- For LangChain: Python ≥ 3.9.
- For Node.js SDK version guidance, see [Common prerequisites — Node.js](/integrations/overview#common-prerequisites--nodejs).
...
const client = new OpenAI({
  ...
  apiKey: process.env.MYCOROUTE_API_KEY,    // never hard-code the key
});
...
Run with:
```bash
export MYCOROUTE_API_KEY='<api-key>'
node script.mjs
```
...
|---|---|---|
| `AuthenticationError` / `401 Unauthorized` | Wrong `api_key`, or `apiKey` resolved to `undefined`/`None` from an unset env var | Verify with `echo $MYCOROUTE_API_KEY`; for SDKs that auto-read `OPENAI_API_KEY` ensure no stale value is shadowing yours |
...
- **No formal Python/JS Mycoroute SDK** — you use the upstream OpenAI / Anthropic SDKs against the router's URL. This is intentional: it keeps you on the most stable, best-documented client libraries; nothing to maintain on the Mycoroute side beyond the OpenAI-compatible endpoint contract.

---

## See also

- [Integrations Overview](/integrations/overview) — when to use code vs Claude Code / OpenCode / pi / Open WebUI.
- [Inference Endpoints](/api/chat-completions) — `/v1/chat/completions` and `/v1/messages` reference and the allowlisted passthrough.
- [auth.yaml Reference](/security/auth-yaml-reference) — API keys, quotas, token budget enforcement, rate-limit headers.
