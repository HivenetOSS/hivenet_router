...
```json
{
  "apiKey": "!pass show services/mycoroute/pi"
}
```
...
`mycoroute` is Pi’s local provider ID.

It is used by:

```bash
pi --provider mycoroute
```
...
```json
{
  ...
  "defaultModel": "mycoroute-code-model"
}
```
