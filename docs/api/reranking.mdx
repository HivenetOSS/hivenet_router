...
Mycoroute routes these requests to agents registered with the `reranker` capability.
...
Mycoroute validates the `model` field. The backend validates the other fields.
...
Mycoroute limits `/v1/*` request bodies to `10485760` bytes, or 10 MiB, by default. The complete query and document array count toward this limit.

Requests above the configured limit receive HTTP `413` before routing. Configure the limit with `mycoroute_MAX_REQUEST_BYTES`, or set it to `0` to disable the built-in check. A reverse proxy or ingress may enforce a smaller limit.
...
The exact fields depend on the backend. Mycoroute returns the successful response without changing result scores or ordering.
...
Mycoroute forwards the original JSON request to the backend.

You can include additional fields supported by the selected reranking service, but Mycoroute does not interpret or normalize them.
...
Mycoroute waits for the selected backend to score the documents and return the complete result set. The endpoint does not use server-sent events.
...
Reranking agents use the same routing pipeline as other Mycoroute workloads.
...
Mycoroute accepts the reranker capability with:
...
<Warning>
  Ollama does not expose the required reranking endpoint. Mycoroute rejects an agent started with both `--engine ollama` and `--capability reranker`.
</Warning>
