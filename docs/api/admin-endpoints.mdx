...
Admin endpoints use Bearer token authentication. Set one or more admin keys (comma-separated) in the `mycoroute_ADMIN_API_KEYS` environment variable before starting the router, and enable admin auth in `auth.yaml`:
...
```bash
export mycoroute_ADMIN_API_KEYS="your-admin-key"
...
> **Dynamic mode is an exception.** When `mycoroute_AUTH_MODE=dynamic`, the router auto-elevates `admin.mode` to `api-key` regardless of `auth.yaml`, and requires `mycoroute_ADMIN_API_KEYS` to be set. Startup fails if it isn't. This guarantees the [API Key Management](#api-key-management) endpoints below are never reachable unauthenticated in dynamic deployments.
...
```bash
# Get routing table
curl -H "Authorization: Bearer $mycoroute_ADMIN_API_KEYS" \
...
- the matching Prometheus series (`mycoroute_agent_requests_success_total`, `..._failed_total`, `mycoroute_agent_success_rate`, `..._disconnections_total`, `mycoroute_agent_connection_resets_total`, `mycoroute_agent_srtt_ms`, input/output tokens, etc.).
...
**Not affected:** tenant/billing counters (`quotaTPD:` and `mycoroute_tenant_*`), agent metadata, liveness gauges (`mycoroute_routing_agent_info` / `_healthy`), and capacity utilization. Routing-level request counters live only in memory and already reset when the router process restarts (e.g. on deploy).
...
```bash
curl -X POST -H "Authorization: Bearer $mycoroute_ADMIN_API_KEYS" \
...
```bash
curl -X PUT -H "Authorization: Bearer $mycoroute_ADMIN_API_KEYS" \
...
```json
{
  "gptoss": {
    ...
    "fallback_chain": [
      {
        "name": "relaxed",
        "match": {"engine": "vllm", "organization": "Mycoroute"},
...
#!/bin/bash
while true; do
  curl -s -H "Authorization: Bearer $mycoroute_ADMIN_API_KEYS" \
...
```bash
# Update policy file
cat > /etc/mycoroute/policy.yaml <<EOF
routing_policy:
  strategy: "least-loaded"
  max_tries: 3
EOF
...
Endpoints for managing the dynamic API key registry at runtime. Only available when `mycoroute_AUTH_MODE=dynamic` — returns `501 Not Implemented` otherwise.
...
- These endpoints are the key-management surface, so admin auth is mandatory in dynamic mode. The router automatically elevates `admin.mode` to `api-key` when `mycoroute_AUTH_MODE=dynamic` and requires `mycoroute_ADMIN_API_KEYS` to be set; startup fails with `auth: admin section mode=api-key but mycoroute_ADMIN_API_KEYS env var is not set` if it's missing. There is no env-var-only configuration path that leaves `/admin/*` public in dynamic mode.
...
- [Chat Completions](/api/chat-completions) - User-facing API
- [Routing Policy](/routing/policy-yaml-reference) - Policy format
- [Prometheus Metrics](/observability/prometheus-metrics) - Metrics endpoint
