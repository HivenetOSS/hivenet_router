...
> calling API key is allowed to use. A key with [per-model quotas](/security/auth-yaml-reference#per-model-quotas)
> sees exactly the models declared in its `quota.per_model` map; a key with a
> non-empty [`models:` allow-list](/security/auth-yaml-reference#model-whitelist)
...
> [`/admin/models` and `/admin/models/:model`](/api/admin-endpoints#models-operator-view)
...
- [Chat Completions](/api/chat-completions) - Use models for inference
- [Embeddings](/api/embeddings) - Embedding generation
- [Reranking](/api/reranking) - Reranking endpoint
- [Admin Endpoints](/api/admin-endpoints) - Administrative APIs
- [Routing Concepts](/routing/routing-concepts) - How models are routed
