...
| `max_occupancy_share` | No | Serverless occupancy-share setting; `0` is unset, otherwise valid through `1` |
...
| `input_tokens_per_minute` | No | Serverless input-token rate limit per key and model |
| `output_tokens_per_minute` | No | Serverless output-token rate limit per key and model |
...
### Serverless key fields

The flat quota shape also accepts:
...
`input_tokens_per_minute` and `output_tokens_per_minute` must be non-negative. They cannot be mixed with `quota.per_model`. `max_occupancy_share` sits on the key entry rather than inside `quota`; it must be `0` (unset) or greater than `0` and at most `1`.

For static keys that can reach a serverless policy, startup also checks that a nonzero `input_tokens_per_minute` is at least that policy's `max_input_tokens`. Policy and auth reloads re-check the same invariant before replacing the active configuration.

At runtime for `mode: serverless` policies:

- `max_occupancy_share` limits the key's simultaneous token-weighted footprint to `share × admit_budget_tokens`. It rejects immediately with `429 rate_limit_exceeded` and `Retry-After: 1`.
- `input_tokens_per_minute` uses a continuously refilling, one-minute-capacity bucket. The learned input estimate is deducted before the daily token budget.
- `output_tokens_per_minute` is deducted after the response. If recent output drains the bucket, the next request returns `429 rate_limit_exceeded` and `Retry-After: 1`. A single response is charged at most one minute of capacity.

These limits do not run for `mode: reserved`, and `0` disables each limit.
