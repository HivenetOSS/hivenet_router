...
func (b *recordingBackend) start(t *testing.T) *httptest.Server {
	...
	record := func(r *http.Request) {
		...
		b.mu.Lock()
		b.calls = append(b.calls, backendCall{path: r.URL.Path, auth: r.Header.Get("Authorization"), xAPIKey: r.Header.Get("X-Api-Key"), body: body})
		b.mu.Unlock()
	}
...
// agent with a backend API key, against a recording backend.
func TestSemanticAliasEndToEnd(t *testing.T) {
	...
	acfg.BackendAPIKey = "sk-backend-secret"
	...
	t.Run("alias resolved and forwarded with backend key", func(t *testing.T) {
		...
		if call.auth != "Bearer sk-backend-secret" || call.xAPIKey != "" {
			t.Errorf("backend credentials: auth=%q x-api-key=%q", call.auth, call.xAPIKey)
		}
	})
	...
	t.Run("anthropic messages via proxy path carries backend key", func(t *testing.T) {
		...
		call := be.last()
		if call.path != "/v1/messages" || call.auth != "Bearer sk-backend-secret" {
			t.Errorf("backend call path=%s auth=%q", call.path, call.auth)
		}
