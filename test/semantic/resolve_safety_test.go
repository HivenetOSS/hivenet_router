...
// TestLoadModelDoc_PinMaxAge: pin_max_age defaults to 4h and must be a
// positive duration; empty strip markers are rejected.
func TestLoadModelDoc_PinMaxAge(t *testing.T) {
	if got := mustAlias(t, pinDoc("auto", "{ model: big }")).PinMaxAgeDuration().String(); got != "4h0m0s" {
		t.Errorf("default pin_max_age = %s", got)
	}
	for _, bad := range []string{
		strings.Replace(pinDoc("auto", "{ model: big }"), "alias:\n", "alias:\n  pin_max_age: nope\n", 1),
		strings.Replace(pinDoc("auto", "{ model: big }"), "alias:\n", "alias:\n  strip_markers: [\"<a>\", \"\"]\n", 1),
	} {
		if _, err := policy.LoadModelDocBytes([]byte(bad)); err == nil {
			t.Errorf("expected a validation error for:\n%s", bad)
		}
	}
}
